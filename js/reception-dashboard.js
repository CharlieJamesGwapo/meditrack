...
function switchSection(section) {
    ...
    // Show selected section or create placeholder
    let targetSection = document.getElementById(`${section}Section`);
    if (!targetSection) {
        // Create placeholder content for sections that don't exist yet
        createPlaceholderSection(section);
    } else {
        targetSection.style.display = 'block';
    }
    
    // Also show the main dashboard content if it exists
    const dashboardContent = document.querySelector('.content-section');
    if (section === 'dashboard' && dashboardContent) {
    ...
    switch(section) {
        case 'dashboard':
            ...
            break;
        case 'checkin':
            // QR check-in section is always visible
            break;
        case 'patients':
            showComingSoon('Patient Management');
            break;
        case 'appointments':
            showComingSoon('Appointments Management');
            break;
        case 'communication':
            showComingSoon('Communication System');
            break;
        case 'account':
...
// Create placeholder section for features not yet implemented
function createPlaceholderSection(section) {
    const mainContent = document.querySelector('.main-content .p-6') || document.querySelector('.main-content');
    if (!mainContent) return;

    // Remove existing placeholder
    const existingPlaceholder = document.getElementById('placeholderSection');
    if (existingPlaceholder) {
        existingPlaceholder.remove();
    }

    // Hide existing content
    const existingContent = mainContent.children;
    for (let child of existingContent) {
        if (child.id !== 'placeholderSection') {
            child.style.display = 'none';
        }
    }

    const placeholder = document.createElement('div');
    placeholder.id = 'placeholderSection';
    placeholder.className = 'content-section';
    placeholder.innerHTML = `
        <div class="text-center py-20">
            <div class="mx-auto w-24 h-24 bg-green-100 rounded-full flex items-center justify-center mb-6">
                <i class="fas fa-cog text-green-600 text-3xl"></i>
            </div>
            <h3 class="text-2xl font-bold text-gray-900 mb-4">Feature Coming Soon</h3>
            <p class="text-gray-600 mb-6">This feature is currently under development.</p>
            <button onclick="switchSection('dashboard')" class="bg-green-600 text-white px-6 py-3 rounded-lg hover:bg-green-700 transition">
...
function showComingSoon(featureName) {
    Swal.fire({
        title: 'Coming Soon!',
        text: `${featureName} feature is under development.`,
        icon: 'info',
